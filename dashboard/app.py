...
class BrainService:
    ...
    def refresh(self):
        ...
        try:
            ...
            with self.lock:
                self.cache = {"status":"Live", "at":datetime.now().strftime("%Y-%m-%d %H:%M:%S UTC"), "frames":frames, "ticker":ticker,"funding":funding,"oi":oi,"fg":fg,"ls":ls,"book":book,"news":news,"macro":macro,"cycle":cycle,"signals":signal_list,"mtf":mtf,"decision":decision,"queued":queued}
...
def create_app(service: BrainService) -> Dash:
    ...
    @app.callback(Output("status","children"),Output("overview","children"),Output("macro","children"),Output("decision","children"),Output("chart","figure"),Output("signals","children"),Output("news","children"),Output("queue","children"),Input("poll","n_intervals"),Input("timeframe","value"),Input("refresh","n_clicks"))
    def render(_, tf, __):
        ...
        return status,overview,macro_ui,decision,fig,sigui,newsui,queue
